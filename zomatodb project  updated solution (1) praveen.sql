...
select max(customer_id),count(order_id) as highest_order
from order_detail
...
select max(restaurant_id), avg(rrating)as highest_avg_rating
from restaurant
...
#4Calculate the total revenue generated by each food item. Display food_id, food_name and Total revenue ordered by total revenue in descending order.

select foods.food_id,foods.food_name,sum(order_food.quantity*foods.price_per_unit)as total_revenue
from foods
...
select foods.food_name, sum(order_food.quantity) as popular_items
from foods
join order_food on foods.food_id = order_food.food_id
...
select employee_name,avg(employee_avg_rating) as highest_rating
...
select  monthname(order_time)as order_month, count(order_id) as total_order
 from order_detail
 ...
 select c.customer_id ,c.customer_name,avg(order_food.quantity* foods.price_per_unit)as avg_order_amount
 from customer c 
 join order_food  on c.customer_id = order_food.customer_id
...
SELECT restaurant_id, restaurant_name, customer_id, customer_name, order_count
FROM (SELECT r.restaurant_id, r.restaurant_name, c.customer_id, c.customer_name, COUNT(o.order_id) AS order_count,
dense_rank() OVER (PARTITION BY r.restaurant_id ORDER BY COUNT(o.order_id) DESC) AS rnk
FROM restaurant r 
JOIN order_detail o ON r.restaurant_id = o.restaurant_id
...
WHERE WEEKDAY(order_time) IN (5, 6);
 ...
 select 
CASE WHEN WEEKDAY(order_time) BETWEEN 0 AND 4 THEN 'Weekday'
ELSE 'Weekend'
...
from foods
group by food_id,food_name,price_per_unit;

# 14 Find the restaurant with the most diverse menu (i.e., the highest number of food items)
SELECT r.restaurant_id, r.restaurant_name, COUNT(order_food.food_id) AS food_item_count
FROM restaurant r
JOIN order_food  ON r.restaurant_id = order_food.restaurant_id
GROUP BY r.restaurant_id, r.restaurant_name
ORDER BY food_item_count DESC
limit 1;

# 15  Calculate total payment amount for each payment type. 
select p.payment_type,sum(ofd.quantity*f.price_per_unit) as total_pay_amount
